...
void loadHash(char* hash, uint32_t* a, uint32_t* b, uint32_t* c, uint32_t* d) {
...
void hexdump(const char * label, const uint32_t * buffer, size_t len) {
   int i;
   ebsp_message("%s: ", label);
   for (i=0; i<len; ++i) {
      ebsp_message("%02x", 0xFF & ((char*)buffer)[i]);
   }
   ebsp_message("\n");
}
...
int main(int argc, char * argv[]) {   
   ...
   int p = bsp_pid();
    ...
    char * hash  = "02c425157ecd32f259548b33402ff6d3";   // zzzz
//   char * hash  = "b870c68f509d670e3eb032349cb2a71a";    // ;;;;
...
//   char * hash        = data->hash;
   ebsp_message("I am core %d/%d and I will explore from %c;;; to %czzz the hash %s \n", p,n, init,stop, hash);
...
// **********************************************************************************************
       ...
       if ( targetA == a0
         ...
         && targetC == c0
         && targetD == d0) {
            ebsp_message("<<< FOUND KEY : %c%c%c%c >>>\n", char0,char1,char2,char3);
            //ebsp_message("hashcount : %lu\n", hashcount);
            bsp_end();
            return 0;
         }
   } 
