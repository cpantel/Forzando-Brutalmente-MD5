...
int main(int argc, char* argv[]) {
  ...
  printf("module pipeline (\n");
  printf("  input  CLK,\n");
  printf("  input  [31:0]counter_in,\n");
  printf("  input  [127:0]target_hash,\n");
  printf("  input  reset,\n");
  printf("  //output [127:0]hash,\n");
  printf("  output reg [31:0]counter_out,\n");
  printf("  output found\n");
  printf(");\n");
  printf("reg [127:0] hash;\n");
  printf("reg hash_equal = 0;\n");
  printf("\n");


for (int i =0; i<64; ++i) {
  printf("reg [31:0] counter%02d, A%02d, B%02d, C%02d, D%02d, F%02d;\n", i,i,i,i,i,i);
}
...
printf("localparam Stage21Value = 32'h2441453;\n");
...
printf("localparam Stage43Value = 32'h4881d05;\n");
...
printf("// stage 00\n");
  ...
  printf("  D%02d = C%02d;\n",y,x);
  printf("  C%02d = B%02d;\n",y,x);
  printf("  B%02d = B%02d + ( (F%02d << %d) | (F%02d >> ( 32 - %d ) ) );\n",y,x,y,z,y,z);
  printf("end\n\n");
  }
  ...
  printf("  A63 = D62 + AValue;\n");
  printf("  D63 = C62 + DValue;\n");
  printf("  C63 = B62 + CValue;\n");
  printf("  B63 = B62 + BValue + ( (F63 << 21) | (F63 >> ( 32 - 21 ) ) );\n");
  printf("end\n");
  ...
  printf("always @(posedge CLK)\n");
  printf("begin\n");
  printf("  hash[127:120] <= A63[ 7: 0];\n");
  printf("  hash[119:112] <= A63[15: 8];\n");
  printf("  hash[111:104] <= A63[23:16];\n");
  printf("  hash[103: 96] <= A63[31:24];\n");
  printf("  \n");
  printf("  hash[ 95: 88] <= B63[ 7: 0];\n");
  printf("  hash[ 87: 80] <= B63[15: 8];\n");
  printf("  hash[ 79: 72] <= B63[23:16];\n");
  printf("  hash[ 71: 64] <= B63[31:24];\n");
  printf("  \n");
  printf("  hash[ 63: 56] <= C63[ 7: 0];\n");
  printf("  hash[ 55: 48] <= C63[15: 8];\n");
  printf("  hash[ 47: 40] <= C63[23:16];\n");
  printf("  hash[ 39: 32] <= C63[31:24];\n");
  printf("  \n");
  printf("  hash[ 31: 24] <= D63[ 7: 0];\n");
  printf("  hash[ 23: 16] <= D63[15: 8];\n");
  printf("  hash[ 15:  8] <= D63[23:16];\n");
  printf("  hash[  7:  0] <= D63[31:24];\n");
  printf("  counter_out <= counter63;\n");
  printf("end\n");
  printf("\n");
  printf("// stage 65\n");
  printf("always @(posedge CLK)\n");
  printf("begin\n");
  printf("  if (hash == target_hash)\n");
  printf("    hash_equal <= 1'h1;\n");
  printf("end\n");
  printf("\n");
  printf("\n");
  printf("sr_ff ff(.CLK(CLK),\n");
  printf("  .set(hash_equal),\n");
  printf("  .reset(reset),\n");
  printf("  .Q(found)\n");
  printf(");\n");
  printf("\n");
