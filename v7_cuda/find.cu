...
__global__ void find( uint32_t hostA, uint32_t hostB, uint32_t hostC, uint32_t hostD, uint32_t * outM, char * msg ) {
   ...
   char chara, char0,char1,char2,char3,char4; 
   int bid = blockIdx.x;
   int tid = threadIdx.x;
   ...
   //printf("I am thread %d (%d x %d) and I will explore from %c%c;;;; to %c%czzzz a hash \n", uid,bid, tid, chara, char0, chara, char0);
...
int main( int argc, char * argv[] ) {
    ...
    // allocate and prepare answer
    char msg[9] = {0,0,0,0,0,0,0,0,0};
    char *dev_msg;

    // allocate and prepare candidat message
    uint32_t M[BUFFER_LEN];
    uint32_t * dev_M;
    pad("012345", M);
